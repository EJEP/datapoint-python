...
class TestForecast(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(5):
            ...
            ts.name = 9 + (3 * i)
            ts.date = datetime.datetime(
                2020, 3, 3, 9 + (3 * i), tzinfo=datetime.timezone.utc
            )
    ...
    def test_at_datetime_1_5_hours_before_after(self):
        target_before = datetime.datetime(
            2020, 3, 3, 7, 0, tzinfo=datetime.timezone.utc
        )

        target_after = datetime.datetime(
            2020, 3, 4, 23, 0, tzinfo=datetime.timezone.utc
        )

        self.assertRaises(
            datapoint.exceptions.APIException,
            self.forecast_3hrly.at_datetime,
            target_before,
        )
    ...
    def test_at_datetime_6_hours_before_after(self):
        ...
        target_before = datetime.datetime(
            2020, 3, 2, 15, 0, tzinfo=datetime.timezone.utc
        )

        target_after = datetime.datetime(2020, 3, 6, 7, 0, tzinfo=datetime.timezone.utc)

        self.assertRaises(
            datapoint.exceptions.APIException,
            self.forecast_daily.at_datetime,
            target_before,
        )
    ...
    def test_normal_time(self):
        target = datetime.datetime(2020, 3, 3, 10, 0, tzinfo=datetime.timezone.utc)

        nearest = self.forecast_3hrly.at_datetime(target)
        expected = datetime.datetime(2020, 3, 3, 9, tzinfo=datetime.timezone.utc)
        self.assertEqual(nearest.date, expected)

        target = datetime.datetime(2020, 3, 3, 11, 0, tzinfo=datetime.timezone.utc)

        nearest = self.forecast_3hrly.at_datetime(target)
        expected = datetime.datetime(2020, 3, 3, 12, tzinfo=datetime.timezone.utc)
        self.assertEqual(nearest.date, expected)

    def test_forecase_midnight(self):
        target = datetime.datetime(2020, 3, 4, 0, 15, tzinfo=datetime.timezone.utc)

        nearest = self.forecast_3hrly.at_datetime(target)
        expected = datetime.datetime(2020, 3, 4, 0, tzinfo=datetime.timezone.utc)
        self.assertEqual(nearest.date, expected)
