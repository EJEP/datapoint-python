...
class TestDataPoint(unittest.TestCase):
    ...
    def setUp(self, mock_request):
        with open(
            "{}/datapoint.json".format(pathlib.Path(__file__).parent.absolute())
        ) as f:
    ...
    def test_wavertree_hourly(self, mock_request):
        ...
        MockDateTime.now = classmethod(
            lambda cls, **kwargs: datetime(2020, 4, 25, 12, tzinfo=timezone.utc)
        )
        mock_request.get(
        ...
        forecast = self.conn.get_forecast_for_site(
            self.wavertree.location_id, "3hourly"
        )
        now = forecast.now()
        ...
        self.assertEqual(
            forecast.days[3].timesteps[7].date.strftime(DATETIME_FORMAT),
    ...
    def test_wavertree_daily(self, mock_request):
        ...
        self.assertEqual(
            forecast.days[4].timesteps[1].date.strftime(DATETIME_FORMAT),
    ...
    def test_kingslynn_hourly(self, mock_request):
        ...
        mock_request.get(
            ...
            text=self.kingslynn_hourly,
        )

        forecast = self.conn.get_forecast_for_site(
            self.kingslynn.location_id, "3hourly"
        )
        now = forecast.now()
        ...
        self.assertEqual(
            forecast.days[4].timesteps[5].date.strftime(DATETIME_FORMAT),
            "2020-04-29 15:00:00+0000",
        )
