...
class ManagerIntegrationTestCase(unittest.TestCase):
    ...
    def test_site(self):
        site = self.manager.get_nearest_forecast_site(
            latitude=51.500728, longitude=-0.124626
        )
    ...
    def test_get_daily_forecast(self):
        ...
        self.assertLess(
            forecast.data_date - datetime.datetime.now(tz=tz),
            datetime.timedelta(hours=3),
        )
        # First forecast should be less than 12 hours away
        tz = forecast.days[0].timesteps[0].date.tzinfo
        self.assertLess(
            forecast.days[0].timesteps[0].date - datetime.datetime.now(tz=tz),
        ...
        for day in forecast.days:
            for timestep in day.timesteps:
                ...
                self.assertEqual(
                    self.manager._weather_to_text(int(timestep.weather.value)),
    ...
    def test_get_nearest_observation_site(self):
        site = self.manager.get_nearest_observation_site(
            longitude=-0.1025, latitude=51.3263
        )
    ...
    def test_get_observation_with_wind_data(self):
        ...
        self.assertLess(
            observation.data_date - datetime.datetime.now(tz=tz),
        ...
        self.assertGreater(
            datetime.datetime.now(tz=tz) - observation.days[0].timesteps[0].date,
        ...
        for day in observation.days:
            for timestep in day.timesteps:
                ...
                self.assertIn(
                    timestep.visibility.text, ["UN", "VP", "PO", "MO", "GO", "VG", "EX"]
                )
